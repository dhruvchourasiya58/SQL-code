...
select *, lead(amount,1) over(), lag(amount,1) over() from orders; -- lag mtlb piche lead mtlb next 


select *, lead(amount,1,0) over(partition by customer_id order by order_date),
lag(amount,2,0) over(partition by customer_id order by order_date),
amount + lag(amount,1,0) over(partition by customer_id order by order_date) + lag(amount,2,0) over(partition by customer_id order by order_date)
 from orders;
 
select customer_id, customer_name, amount,
sum(amount) over( rows between 2 preceding and current row) from orders; -- running sum show krne k liye humesha rows between use hoga
