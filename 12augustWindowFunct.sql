...
select customer_id, order_id, amount, row_number() over(order by amount desc) from orders;

select category, order_id, amount, rank() over(partition by category order by amount desc) from orders;

select order_id, amount,
avg(amount) over(partition by category) as category_avg,
round( amount - avg(amount) over(partition by category) ) as difference from orders;

-- 1st method

select * from ( select customer_id, customer_name, order_id, amount,
max(amount) over(partition by customer_id) as customer_max from orders ) as t
...
-- Find 2nd highest- value orders for every customer. 

select * from ( select customer_id, customer_name, order_id, amount,
rank() over(partition by customer_id order by amount desc) as customer_rank from orders ) as t
where customer_rank = 2;

-- Find 2 highest- value orders for every customer.

select * from ( select customer_id, customer_name, order_id, amount,
rank() over(partition by customer_id order by amount desc) as customer_rank from orders ) as t
where customer_rank <= 2;

-- calculate the percentage contribution of each order to the customer's total sepnding ?

select customer_name, order_id, amount, 
sum(amount) over(partition by customer_id) as total_spending,
round(amount/ sum(amount) over(partition by customer_id) *100, 2) from orders; -- 100, 2 isliye taki % me '.' k baad 2 digit show ho 
 
-- next topic leg, lead, ntile, row number( important runnung sum / running average, last 3 orders imformation )