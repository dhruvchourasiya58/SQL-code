...
-- Q. Employees who are not the highest- paid employee in their department.

select emp_name from employee as eout where 
salary != (select max(salary) from employee where dept_id = eout.dept_id);


-- Q. Departments having at least one employee earing more than 70000?

select dept_name, dept_id from department where
dept_id = any(select dept_id from employee
where salary > 70000);

-- Q. Second - Highest - paid employee in each department?

select emp_name, dept_id, salary from employee as e1 where 
salary = ( select max(salary) from employee as e2 
